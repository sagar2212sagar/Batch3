...
# MAGIC
# MAGIC SYSTEM_PROMPT = """You are a recruitment assistant with access to two tools:
# MAGIC  
# MAGIC 1. search_resumes_final      — find candidates by skills / role
# MAGIC 2. search_job_descriptions _final— find open roles by requirements
# MAGIC  
# MAGIC When asked to match a candidate to jobs, first search for JDs with the candidate's
# MAGIC skills. When asked to find candidates for a role, search resumes with the JD
# MAGIC requirements. Always explain your reasoning and rank results by relevance"""
# MAGIC
# MAGIC
# MAGIC ###############################################################################
...
# MAGIC UC_TOOL_NAMES = ["resume_batch3.gold.search_job_descriptions_final", "resume_batch3.gold.search_resumes_final"]
# MAGIC
...
# MAGIC             index_name="resume_batch3.gold.resume_data_index",
...
# MAGIC VECTOR_SEARCH_TOOLS.append(
# MAGIC         VectorSearchRetrieverTool(
# MAGIC             index_name="resume_batch3.gold.company_jd_index",
# MAGIC             # TODO: specify index description for better agent tool selection
# MAGIC             # tool_description=""
# MAGIC         )
# MAGIC     )
...
input_example = {
    "input": [
        {
            "role": "user",
            "content": "give me best profile for data engineer"
        }
    ],
    "custom_inputs": {
    ...
    },
    {
        "inputs": {
            "input": [
                ...
                {
                    "role": "assistant",
                    "content": "I am using this tool to find the best profiles for a data engineer role. The tool will allow me to search through resumes and find the top matches based on the query 'data engineer'.\n\n<uc_function_call>{\"id\":\"rbr29v0rGjwBssu8rnNZ\",\"name\":\"resume_batch3.gold.search_resumes_final\",\"arguments\":\"{\\\"query\\\":\\\"data engineer\\\"}\"}</uc_function_call>\n\n<uc_function_result>{\"id\":\"rbr29v0rGjwBssu8rnNZ\",\"content\":\"{\\\"is_truncated\\\":false,\\\"columns\\\":[\\\"resume_id\\\",\\\"name\\\",\\\"experience\\\",\\\"skills\\\"],\\\"rows\\\":[[\\\"5\\\",\\\"Meera Krishnan\\\",\\\"14+\\\",\\\"Python Scala Java Spark Trino Iceberg Cost Optimization Data Platform Architecture\\\"],[\\\"1\\\",\\\"Daniel Park\\\",\\\"8\\\",\\\"Python Scala Spark Kafka Flink AWS EMR Kinesis DynamoDB Elasticsearch OpenSearch Airflow Kubernetes Real-time Analytics Clickstream Processing\\\"],[\\\"12\\\",\\\"Sagar Prajapati\\\",\\\"6+\\\",\\\"Python, PySpark, Spark SQL, Delta Lake, CI/CD automation, data quality frameworks, MLOps with MLflow\\\"]]}\"}</uc_function_result>\n\nBased on the search results, the best profile for a data engineer is Meera Krishnan, with 14+ years of experience and skills in Python, Scala, Java, Spark, Trino, Iceberg, cost optimization, data platform architecture. This profile stands out due to the combination of technical skills and experience in data engineering, which are essential for designing, building, and maintaining large-scale data systems. \n\nHowever, it's also important to consider the specific requirements of the role you're looking to fill, as different data engineering positions may prioritize different skills and experiences. For example, if you're looking for someone with expertise in real-time analytics, Daniel Park's profile may be more suitable. \n\nIt's always a good idea to review the resumes and cover letters of the top candidates and conduct interviews to get a better sense of their qualifications and fit for the role."
                },
                {
                    "role": "user",
                    "content": "what is the best JD for Sagar Prajapati"
                }
            ]
        },
        "expected_response": None
...
catalog = "resume_batch3"
schema = "gold"
model_name = "agent_hr_final"
